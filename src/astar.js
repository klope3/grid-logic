...
function findPath(grid, start, end) {
    ...
    while (true) {
        const neighbors = getNeighborCells(cellGrid, current.coordinates).filter((n) => n && !n.visited && (n.walkable || n === endCell));
        for (const n of neighbors) {
            if (!n)
                continue;
            n.hCost = distance(n.coordinates, end);
            const wouldBeGCost = n.gCost + distance(current.coordinates, n.coordinates);
...
function initCells(grid) {
    const cells = Array.from({ length: grid.length }, (_) => Array.from({ length: grid[0].length }, (_) => ({})));
...
function getNeighborCoords(grid, position) {
    const width = grid[0].length;
    const height = grid.length;
    return {
        left: position.x > 0 ? { x: position.x - 1, y: position.y } : undefined,
        top: position.y > 0 ? { x: position.x, y: position.y - 1 } : undefined,
        right: position.x < width - 1 ? { x: position.x + 1, y: position.y } : undefined,
        bottom: position.y < height - 1
            ? { x: position.x, y: position.y + 1 }
            : undefined,
    };
}
function getNeighborCells(cellGrid, centerCoords) {
    const { left, top, right, bottom } = getNeighborCoords(cellGrid, centerCoords);
    const neighborCells = [
...
function addCellToOpen(open, cellToAdd) {
    if (open.includes(cellToAdd))
        return;
    open.push(cellToAdd);
    open.sort((a, b) => a.gCost + a.hCost - (b.gCost + b.hCost));
}
function removeCellFromOpen(open, cellToRemove) {
